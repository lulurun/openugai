...
sub _simulator_login {
	...
	my %response = ();
	if ($params->{"UUID"}) {
		$region_data = &OpenUGAI::GridServer::GridManager::getRegionByUUID($params->{"UUID"});
...
sub _simulator_data_request {
	...
	if ($params->{"region_UUID"}) {
		$region_data = &OpenUGAI::GridServer::GridManager::getRegionByUUID($params->{"region_UUID"});
	} elsif ($params->{"region_handle"}) {
		$region_data = &OpenUGAI::GridServer::GridManager::getRegionByHandle($params->{"region_handle"});
	}
	if (!$region_data) {
...
sub _map_block {
	...
	my @sim_block_list = ();
	my $region_list = &OpenUGAI::GridServer::GridManager::getRegionList($xmin, $ymin, $xmax, $ymax);
	foreach my $region (@$region_list) {
		my %sim_block = (
			"x" => $region->{locX},
			"y" => $region->{locY},
...
sub _map_block2 {
	...
	my @sim_block_list = ();
	my $region_list = &OpenUGAI::GridServer::GridManager::getRegionList2($xmin, $ymin, $xmax, $ymax);
	foreach my $region (@$region_list) {
		my %sim_block = (
			"x" => $region->{locX},
			"y" => $region->{locY},
