...
use OpenUGAI::AssetServer::AssetManager;
...
sub getAsset {
	my ($asset_id, $param) = @_;
	# get asset
	my $asset_id_string = &OpenUGAI::Utility::UUID2HEX($asset_id);
	my $asset = &OpenUGAI::AssetServer::AssetManager::getAssetByUUID($asset_id_string);
...
sub saveAsset {
	...
	&OpenUGAI::AssetServer::AssetManager::saveAsset($asset);
...
print STDERR $obj->{FullID}->{UUID} . "\n";
	my %asset = (
		"id" => &OpenUGAI::Utility::UUID2BIN($obj->{FullID}->{UUID}),
		"name" => $obj->{Name},
