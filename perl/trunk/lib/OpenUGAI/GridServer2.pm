...
sub StartUp {
    $OpenUGAI::Global::Region_Dir = "/var/www/openugai/data/Regions";
    $OpenUGAI::Global::Contents_Dir = "/var/www/openugai/perl/trunk/Contents";
...
sub init {
    ...
    Apache2::ServerRec::warn("grid2 server RegionDIR " . $OpenUGAI::Global::Region_Dir);
    Apache2::ServerRec::warn("grid2 server ContentsDIR " . $OpenUGAI::Global::Contents_Dir);
    ...
    Apache2::ServerRec::warn("grid2 server initialized $$");
}
...
sub _get_region_list_handler {
    my ($this, $cgi) = @_;
    my $region_list = &OpenUGAI::DynamicGridHandler::GetRegionList();
    print $cgi->header( -type => 'text/plain', -charset => "utf-8" ), JSON::to_json($region_list);
}
...
sub _get_contents_info_handler {
    my ($this, $cgi, $id) = @_;
    my $contents_info = &OpenUGAI::DynamicGridHandler::GetContentsInfo($id);
    print $cgi->header( -type => 'text/plain', -charset => "utf-8" ), JSON::to_json($contents_info);
}

sub _get_contents_data_handler {
    my ($this, $cgi, $id) = @_;
    my $contents_url = &OpenUGAI::DynamicGridHandler::GetContentsUrl($id);
    &OpenUGAI::DynamicGridHandler::UpdateContentsInfo($id);
