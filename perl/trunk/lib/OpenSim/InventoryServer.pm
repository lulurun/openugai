...
sub _create_default_inventory {
    ...
    push @InventoryFolders, &__create_folder_struct($root_folder_id, $uuid, &OpenSim::Utility::ZeroUUID(), "My Inventory", 8, 1);
    push @InventoryFolders, &__create_folder_struct(&OpenSim::Utility::GenerateUUID(), $uuid, $root_folder_id, "Textures", 0, 1);
    push @InventoryFolders, &__create_folder_struct(&OpenSim::Utility::GenerateUUID(), $uuid, $root_folder_id, "Objects", 6, 1);
    push @InventoryFolders, &__create_folder_struct(&OpenSim::Utility::GenerateUUID(), $uuid, $root_folder_id, "Clothes", 5, 1);
    push @InventoryFolders, &__create_folder_struct(&OpenSim::Utility::GenerateUUID(), $uuid, $root_folder_id, "Bodyparts", 13, 1);
    if ($save_flag) {
	foreach(@InventoryFolders) {
	    &OpenSim::InventoryServer::InventoryManager::saveInventoryFolder(&_convert_to_db_folder($_));
	}
    }
