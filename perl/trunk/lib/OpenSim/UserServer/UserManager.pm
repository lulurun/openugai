...
sub getUserByName {
    my ($first, $last) = @_;
    my $res = &OpenSim::DBData::getSimpleResult($OpenSim::UserServer::Config::SYS_SQL{select_user_by_name}, $first, $last);
    my $count = @$res;
    my %user = ();
    if ($count == 1) {
	my $user_row = $res->[0];
...
sub getUserByUUID {
    my ($uuid) = @_;
    my $res = &OpenSim::DBData::getSimpleResult($OpenSim::UserServer::Config::SYS_SQL{select_user_by_uuid}, $uuid);
    my $count = @$res;
    my %user = ();
    if ($count == 1) {
	my $user_row = $res->[0];
